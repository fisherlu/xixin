...
class ProfilePage extends StatelessWidget {
  ...
  Widget _achievementSection(BuildContext ctx, ProfileProvider pp, ThemeData t) {
    ...
    final locked = pp.lockedAchievements;
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text("成就勋章", style: t.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)),
        Text("${achievements.length}/${AchievementLibrary.list.length}", style: t.textTheme.bodySmall?.copyWith(color: AppColors.textSecondary)),
      ]),
      const SizedBox(height: 4),
      ClipRRect(
        borderRadius: BorderRadius.circular(2),
        child: LinearProgressIndicator(value: pp.achievementProgress, minHeight: 4, backgroundColor: t.cardColor, valueColor: const AlwaysStoppedAnimation(AppColors.accent)),
      ),
      const SizedBox(height: 16),
      Wrap(spacing: 8, runSpacing: 8, children: [
        ...AchievementLibrary.list.map((a) {
          final unlocked = achievements.any((ua) => ua.id == a.id);
          return Container(
            width: (MediaQuery.of(ctx).size.width - 64) / 4,
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), color: t.cardColor, border: unlocked ? Border.all(color: a.color.withOpacity(0.3)) : null),
            child: Column(children: [
              Icon(a.icon, color: unlocked ? a.color : AppColors.textSecondary.withOpacity(0.3), size: 24),
              const SizedBox(height: 8),
              Text(a.title, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: unlocked ? null : AppColors.textSecondary.withOpacity(0.3)), textAlign: TextAlign.center),
            ]),
          );
        }),
      ]),
      ...
      Card(
        child: Column(children: [
          SwitchListTile(
            ...
            value: pp.reminderEnabled,
            onChanged: pp.toggleReminder,
          ),
          const Divider(height: 1),
          ListTile(
            title: const Text("关于正念冥想"),
